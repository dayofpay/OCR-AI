# Text Extractor From Image ( Optical Character Recognition (OCR) using Tesseract)


This is a simple Python program that uses the Tesseract OCR engine to extract text from an image and then filters out invalid words based on a given configuration.
...
# Installation
    ...
    Install OpenCV by running pip install opencv-python in the command prompt or terminal.
    Install pytesseract by running pip install pytesseract in the command prompt or terminal.
    Install Tesseract OCR engine from the official website: https://github.com/UB-Mannheim/tesseract/wiki

# Usage
    Run the script in the terminal using the command python main.py
    Enter the language code of the language pack you want to use (e.g. eng for English, bul for Bulgarian).
    Enter the path of the image you want to extract text from.
    The program will print all invalid words followed by all valid words.
